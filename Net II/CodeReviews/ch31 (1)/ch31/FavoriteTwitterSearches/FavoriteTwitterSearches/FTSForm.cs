   ...
   public partial class FTSForm : Form
   {
         ...
         object sender, EventArgs e )
      {
         ...
         else
         {
            ...
            deleteQueryLabel.Enabled = true; //Enable upon the clicking of manage searches
            deleteQueryButton.Enabled = true; //Enable button upon the clicking of manage searches
            ...
            if ( !groupTagTextBox.Text.Equals( groupTag ) )
            {
               ...
               tagsListBox.Items.Clear(); // clear tagsListBox
      ...
      private void deleteQueryButton_Click(object sender, EventArgs e)
      {
          try
          {
              if (tagsListBox.SelectedItem == null) //Ensures that a query gets selected
              {
                  MessageBox.Show("Please select a query to delete.", "No Selection Made");
              }
              else
              {
                  ListBox.SelectedObjectCollection selectedTags = new ListBox.SelectedObjectCollection(tagsListBox);
                  selectedTags = tagsListBox.SelectedItems;
                  for (int i = selectedTags.Count - 1; i >= 0; i--)
                  {
                      tagsListBox.Items.Remove(selectedTags[i]);
                  }
                  tagsListBox.Refresh(); //refresh after deleting a query
              } //I had a hard time figuring out how to get this to work, and even then still couldn't get it to delete a query.
          }
          catch (Exception) //prevents possible issues with deleting
          { 
