...
// Code has been edited by Anthony Fuller
using System;
...
namespace FibonacciAsynchronous
{
   public partial class AsynchronousTestForm : Form
   {
      ...
      private async void startButton_Click( object sender, EventArgs e )
      {
          ...
          try
          {
              num = Convert.ToInt32(txtBoxValue.Text);
              if (num < 1 || num > 100) {
                  throw new Exception();
              }
          }
         ...
         outputTextBox.AppendText(
            "Starting Task to calculate Fibonacci(" + (num + 1) + ")\r\n" );

         // create Task to perform Fibonacci(num + 1) calculation in a thread
         Task< TimeData > task1 =
            Task.Run( () => StartFibonacci( num + 1 ) );

         outputTextBox.AppendText(
            "Starting Task to calculate Fibonacci(" + num + ")\r\n" );
            
         // create Task to perform Fibonacci(num) calculation in a thread
         Task< TimeData > task2 =
            Task.Run( () => StartFibonacci( num ) );
